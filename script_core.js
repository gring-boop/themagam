...
/* =====================================================================
   ...
   **있는 방을 새 창으로 열어주기만** 합니다.
   ...
   ★ 창 크기를 지정하지 않습니다. 일부러입니다.

   처음엔 width/height 를 줬는데, 크기를 지정하면 브라우저가 이걸
   "팝업 창"으로 해석합니다. 팝업 창에는 주소창도 탭도 닫기 버튼도
   없어서, 앱으로 설치해 쓰는 분들이 **창을 닫을 방법이 없었습니다.**
   (⌘W 를 알아야만 닫혔어요.)

   크기를 안 주면 평범한 새 탭으로 열립니다. 조금 커도 닫을 수 있는
   편이 낫습니다.
...
function openSharedPomo() {
  try {
    ...
    _sharedPomoWin = window.open(
      SHARED_POMO_URL, "sharedPomo", "noopener,noreferrer"
    );
    if (!_sharedPomoWin) {
